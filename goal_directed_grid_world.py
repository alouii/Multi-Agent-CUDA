...
def initialize_agents_and_goals(num_agents, grid_size):
    """Initialize agent positions and goal positions randomly on the grid."""
    # Agent positions: [x, y]
    ...
    # Goal positions: [x, y]
    goal_positions = cp.zeros((num_agents, 2), dtype=cp.int32)
...
def compute_manhattan_distances(agent_positions, goal_positions):
    """Compute Manhattan distance from each agent to its goal."""
...
def choose_directions(agent_positions, goal_positions, grid_size):
    """Choose direction (up, down, left, right) to minimize Manhattan distance to goal."""
    directions = cp.zeros(NUM_AGENTS, dtype=cp.int32)  # 0=up, 1=down, 2=left, 3=right
    
    # Compute distances for each possible move
    new_positions = cp.zeros((NUM_AGENTS, 4, 2), dtype=cp.int32)  # [agent, direction, (x,y)]
    ...
    distances = cp.zeros((NUM_AGENTS, 4), dtype=cp.int32)
    for d in range(4):
...
def check_collisions_and_update(agent_positions, new_positions, directions):
    ...
    for i in range(NUM_AGENTS):
        ...
        grid[x, y] = i + 1  # Agent index + 1 (0 is empty)
    
    # Check for collisions
    valid_moves = cp.ones(NUM_AGENTS, dtype=cp.bool_)
    for i in range(NUM_AGENTS):
        d = directions[i]
        new_x, new_y = new_positions[i, d]
...
def main():
    ...
    print(f"Initialized {NUM_AGENTS} agents with goals on a {GRID_SIZE}x{GRID_SIZE} grid.")
